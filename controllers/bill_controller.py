...
from ..services import bill_detail_service, bill_service
...
def get_bills_by_customer(customerId: int):
    bills = bill_service.getBillsByCustomerId(customerId)
    return jsonify([_serialize_bill(bill) for bill in bills])


@bill_bp.get("/room/<int:roomNumber>")
def get_bills_by_room_number(roomNumber: int):
    bills = bill_service.getBillsByRoomId(roomNumber)
    return jsonify([_serialize_bill(bill) for bill in bills])


@bill_bp.get("/room-id/<int:roomId>")
def get_bills_by_room_id(roomId: int):
    bills = bill_service.getBillsByRoomId(roomId)
    return jsonify([_serialize_bill(bill) for bill in bills])


@bill_bp.get("/unpaid")
def get_unpaid_bills():
    bills = bill_service.getUnpaidBills()
    return jsonify([_serialize_bill(bill) for bill in bills])
...
def pay_bill(billId: int):
    try:
        bill = bill_service.markBillPaid(billId)
...
def cancel_bill(billId: int):
    try:
        bill = bill_service.cancelBill(billId)
...
def print_bill(billId: int):
    ...
    bill_service.markBillPrinted(billId)
    payload = bill_service.buildPrintablePayload(bill, details)
